...
esc_dataset = ESCDataset(download=False, data_size=100) if APPROACH!=2 else ESCDatasetBin(download=False, train_index= list(range(2744)))
...
models = generate_models(FEATURES, ConvolutionalRNNZhang, "crnn_zhang")
...
for model in models:
    print(f"Training model : {model.full_name}")
